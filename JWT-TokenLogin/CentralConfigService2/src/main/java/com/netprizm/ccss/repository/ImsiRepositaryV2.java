...
@Repository
public interface ImsiRepositaryV2 extends JpaRepository<Imsi, Integer> {
...
//	 List<Imsi> findByIsreserved(Boolean isreserved);
	 ...
	 @Transactional
	 @Modifying
	 @Query(value = "update cfg.imsi set environment=?,isreserved=?,runid=? , count =? where imsiid=?",nativeQuery = true)
	 void updateImsi(String environment,Boolean isreserved,Integer runid, Integer count,Integer imsiid);
